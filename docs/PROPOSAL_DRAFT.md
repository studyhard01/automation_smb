...
6. 온프레미스 LLM이 strict `proposal-document-v2` JSON을 출력한다.
7. 서버가 Pydantic으로 추가 필드, 타입, 표 열 수, 인용 ID와 유형별 section 계약을 검증·정규화한다.
8. 별도 온프레미스 검증 호출이 문단·목록 항목·표 행을 `supported`, `derived`, `unsupported`, `conflicting`으로 판정한다.
9. 근거 없는 선택 내용은 삭제하고, 결재에 필수인 누락·충돌만 서로 겹치지 않는 최대 3개 확인 질문으로 바꾼다.
10. 질문이 있으면 XLSX 생성과 SMB 저장을 보류한다. 사용자가 한 번에 답하면 답변을 사용자 근거로 재작성·재검증한다.
11. 확인이 끝난 V2만 기존 세 필드로 투영하고 XLSX에 렌더링한 뒤 SMB에 신규 추가하고 다운로드 ID를 발급한다.
