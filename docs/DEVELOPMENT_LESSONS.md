...
- 상황: DB 조회만 하던 LAN Playground에 제한된 SMB 첨부와 로그인·사용자 권한처럼 서로 다른 쓰기 책임이 추가됐다.
- 교훈: SMB 쓰기는 명시적 활성화·share 하위 경로·비덮어쓰기로 좁히고, 인증 쓰기는 별도 schema·계정·실패 격리로 분리해 조회 adapter의 자격증명과 수명주기를 공유하지 않는다.
- 다음 적용: 새 쓰기 기능은 대상 저장소·최소 권한 계정·초기화 실패 범위·감사 주체를 먼저 고정하고, 기존 읽기 흐름 회귀와 자기 잠금·마지막 관리자 같은 복구 불가 상태를 함께 시험한다.
