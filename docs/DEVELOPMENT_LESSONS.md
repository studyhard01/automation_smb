...
### 2026-07-22 — 외부 tool 탐색과 실행 경로를 분리한다
- 상황: Langflow MCP `tools/list`를 채팅마다 호출하면 외부 서비스 지연·장애가 내장 tool 목록과 fast path까지 전파될 수 있었다.
- 교훈: 외부 tool 계약은 명시적 동기화 때 로컬 스냅샷으로 저장하고, 실제 선택된 tool의 실행만 원격 호출한다.
- 다음 적용: 새 원격 tool 연동도 discovery/control path와 call path를 분리하고 동기화·실행 지연을 각각 측정한다.
