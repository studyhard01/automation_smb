...
- 상황: DB 조회만 하던 LAN Playground에 공유폴더 파일 첨부, 사용자가 바꾸는 저장 경로와 업로드 직후 대화 기능이 추가됐다.
- 교훈: 쓰기는 명시적 활성화·share 내부 상대 경로·크기/확장자 제한·비덮어쓰기로 좁히고, 저장 완료·전역 인덱싱·즉시 대화 준비를 서로 다른 계약으로 표시해야 한다.
- 다음 적용: LAN 쓰기 기능은 인증·권한·감사 승인 전 최소 범위로 유지하고, 저장명 정규화·날짜/초기 버전·traversal·동명이름·초과 크기와 `selected_file` 자동 선택·서버 UUID 재검증을 회귀 테스트한다.
