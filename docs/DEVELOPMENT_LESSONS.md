...
- 상황: 오래된 정리 목록이 이후 활성화된 파일까지 제거 대상으로 기록했고, 완료 계획·설문·중복 설명이 현재 계약 문서와 섞여 실행 경로와 문서 역할을 모두 흐렸다.
- 교훈: 과거 inventory는 실행 계획이 아니다. app router·import·테스트·UI 호출로 도달 가능성을 다시 확인하고, 문서는 현재 상태·기능 계약·운영·로드맵마다 단일 진실 원천을 둔다.
- 다음 적용: 정리 전 기능표와 import graph를 만들고 환경·데이터는 제외한다. 코드·테스트·의존성·문서를 함께 제거한 뒤 색인·깨진 링크·package discovery·pytest·Vite·lock·품질 rubric·Docker smoke를 검증한다.
