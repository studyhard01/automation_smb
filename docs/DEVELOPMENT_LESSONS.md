...
- 상황: GUI에서 보이는 PuTTY 저장 세션도 사용자명·키·host-key 신뢰가 완전하지 않으면 batch 접속이 중단될 수 있었다.
- 교훈: 세션 존재와 비대화식 접속 가능 여부를 분리하고 최초 신뢰·인증 조건을 read-only 명령으로 확인한다.
- 다음 적용: tunnel/점검 스크립트는 session 탐색, 인증 실패, port 충돌을 구분해 안전한 오류로 보고한다.
