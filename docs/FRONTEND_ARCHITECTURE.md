...
파일 첨부와 설정은 실제 Backend API에 연결한다. 첨부 성공 응답의 `selected_file`은 즉시 `대화 참고 파일` 상태에
추가하고 중앙 채팅 요청에 전달한다. 대화 이력과 범용 관리자 메뉴는 현재 제품 경계에서 제외하며, 화면 유사도를 위한
가짜 버튼이나 rule-based 결과를 만들지 않는다.
