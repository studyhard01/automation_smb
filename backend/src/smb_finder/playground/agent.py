...
class PlaygroundAgent:
        ...
        if request.selected_files:
            if request.provider != "local":
                return self._error_response(
                    ...
                    started,
                    request.model,
                    "external_llm_blocked_for_dataset",
                    "선택 문서의 근거는 온프레미스 local LLM에서만 처리할 수 있습니다.",
                    warnings=warnings,
            ...
            return self._run_scoped_llmops_fast_path(
                request=request,
    ...
    def _run_scoped_llmops_fast_path(
        self,
        *,
        request: ChatRequest,
        retriever: Any,
        ...
        """선택 UUID pair로 SQL scope를 강제하고 근거가 있을 때만 local LLM을 호출한다."""
        ...
        artifacts = [
            ArtifactLink(
                ...
                preview_url=(
                    f"/api/playground/files/{scope.doc_id}/revisions/{scope.revision_id}/artifacts/preview"
                ),
                canonical_url=(
                    f"/api/playground/files/{scope.doc_id}/revisions/{scope.revision_id}/artifacts/canonical"
                ),
        ...
        trace = ToolCallTrace(
            ...
            tool_name="선택 문서 근거 검색",
            arguments_summary=(
                f"query_len={len(request.message)} scope_count={len(selections)} "
                f"result_count={retrieval.result_count} elapsed_ms={retrieval.elapsed_ms}"
            ),
            status="ok",
            elapsed_ms=retrieval.elapsed_ms,
            result_text=f"선택 문서 근거 {retrieval.result_count}건",
            result_payload={
        ...
        steps = [
            AgentStepTrace(
                ...
                kind="tool_call",
                title="선택 문서 범위 검색",
                detail=f"문서 {len(selections)}개, 근거 {retrieval.result_count}건",
                action="tool_call",
        ...
        if not citations:
            ...
            steps.append(
                AgentStepTrace(
                    ...
                    kind="final",
                    title="검색 근거 없음",
                    detail="근거 부족으로 생성 LLM을 호출하지 않았습니다.",
                    action="final_answer",
                )
            )
            return ChatResponse(
                ...
                session_id=session_id,
                provider_used="local",
                model_used=request.model or self.settings.llm_model or self.settings.llmops_chat_model,
        ...
        connection = self._resolve_llm_connection(
            provider="local",
            local_base_url=request.local_base_url,
            model=request.model or self.settings.llmops_chat_model,
        )
        if connection.error_code:
            return self._error_response(
                session_id,
        ...
        usage_calls: list[TokenUsage] = []
        debug_calls: list[LlmDebugCall] = []
        per_citation_chars = max(200, self.settings.rag_synthesis_evidence_chars // max(1, len(citations)))
        evidence = [
            {
        ...
        try:
            synthesis = self._chat_json(
                [
                    {
                        "role": "system",
                        "content": (
                            "제공된 온프레미스 문서 근거만 사용해 한국어로 답하세요. "
                            "주요 주장 뒤에 반드시 [근거 index]를 표시하고 근거가 부족하면 그 사실을 말하세요. "
                            "문서 경로나 저장소 정보를 만들지 마세요. JSON {\"answer\":\"...\"}만 반환하세요."
                        ),
                    },
                    {
                        "role": "user",
                        "content": json.dumps(
                            {"question": request.message, "evidence": evidence},
                            ensure_ascii=False,
                        ),
                    },
                ],
                ...
                purpose="llmops_scoped_grounded_synthesis",
                provider="local",
                session_id=session_id,
                request_id=request_id,
                usage_calls=usage_calls,
            ...
            if not answer:
                raise LlmResponseFormatError("Scoped synthesis response has no answer")
        except LlmResponseFormatError:
            return ChatResponse(
                request_id=request_id,
                session_id=session_id,
                ...
                token_usage=self._aggregate_token_usage(usage_calls, "local", connection.model),
            )
        except Exception:
            return ChatResponse(
                request_id=request_id,
                ...
                provider_used="local",
                model_used=connection.model,
                assistant_message="근거 답변을 생성하는 local LLM 호출이 실패했습니다.",
                active_skill_ids=[skill.id for skill in active_skills],
                tool_calls=[trace],
                agent_steps=steps if request.debug_trace else [],
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
        ...
        return ChatResponse(
            ...
            retrieval=retrieval,
            artifacts=artifacts,
            debug=PlaygroundDebug(llm_calls=debug_calls) if debug_allowed and debug_calls else None,
            token_usage=self._aggregate_token_usage(usage_calls, "local", connection.model),
        )
    ...
    def _run_rag_grounded_fast_path(
        ...
        *,
        ...
        selected_labels = [item.title or item.file_name for item in request.selected_files]
        scoped_query = " ".join([*selected_labels, request.message]).strip()[:2000]
        arguments = {"query": scoped_query, "limit": self.settings.rag_db_default_limit}
        trace_started = time.perf_counter()
        try:
            result = self._execute_handler_traced(handler, arguments)
                        ...
                        "content": json.dumps(
                            {
                                ...
                                "selected_files": [
                                    item.model_dump(mode="json", exclude_none=True) for item in request.selected_files
                                ],
    ...
    def _decide_next_action(
        ...
        *,
        ...
        system_prompt = (
            ...
            "When selected_files is not empty, treat those files as the user's explicit document scope. "
            "Use their file names and identifiers to keep retrieval and the final answer within that scope. "
    ...
    def _chat_ollama_json(
        ...
        *,
        ...
        session_id: str,
        request_id: str,
        usage_calls: list[TokenUsage] | None,
        debug_calls: list[LlmDebugCall] | None,
    ) -> dict[str, Any]:
        """Qwen3의 thinking token이 JSON 본문을 잠식하지 않도록 Ollama native API를 쓴다."""

        root_url = self.settings.ollama_base_url.strip().rstrip("/")
        if not root_url:
            raise LlmResponseFormatError("Ollama native base URL is not configured")
        request_messages = _json_request_messages(messages, provider=provider, model=model)
        answer_schema = {
            "type": "object",
        ...
        try:
            ...
            with self.trace_observer.span(
                ...
                attributes={
                    ...
                    "llm.model": model,
                    "llm.purpose": purpose,
                    "llm.api_style": "ollama_native",
                },
            ) as span:
                response = self._llm_http_client().post(f"{root_url}/api/chat", json=payload)
                response.raise_for_status()
                response_json = response.json()
                prompt_tokens = self._usage_int(response_json, "prompt_eval_count")
                completion_tokens = self._usage_int(response_json, "eval_count")
            ...
            if usage is not None and usage_calls is not None:
                usage_calls.append(usage)
            raw_value = response_json.get("message", {}).get("content")
            raw_content = raw_value if isinstance(raw_value, str) else ""
            if not raw_content.strip():
                raise LlmResponseFormatError("Ollama response content is empty")
            try:
                data, repaired = _json_from_text_with_meta(raw_content)
            except (json.JSONDecodeError, TypeError) as exc:
                raise LlmResponseFormatError("Ollama response content is not a JSON object") from exc
            if debug_calls is not None:
                debug_calls.append(
                    LlmDebugCall(
                        purpose=purpose,
    ...
    def _local_base_url(self, override: str = "") -> str:
        return (override or self.settings.effective_ollama_openai_base_url).strip().rstrip("/")

    def _local_api_key(self, base_url: str) -> str:
        configured_base_url = self.settings.effective_ollama_openai_base_url.strip().rstrip("/")
