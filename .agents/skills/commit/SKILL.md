---
name: commit
description: 현재 작업물을 안전하게 git 커밋한다. 사용자가 "/commit", "커밋해줘", "commit 해줘"라고 하거나 현재 변경사항 커밋을 요청할 때 사용한다. status와 diff를 확인하고, 민감정보와 생성 캐시를 차단하며, 커밋 메시지에는 변경 사항에 대한 간단한 한국어 설명과 로컬 커밋 날짜를 넣는다.
---

# /commit 안전 커밋

현재 저장소의 변경사항을 안전하게 커밋한다. 이 프로젝트는 사내 SMB 공유폴더와 민감 의료 데이터를 다룰 수 있으므로, 커밋 전 보안 점검을 반드시 수행한다.
