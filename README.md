...
LLMOps 데이터셋을 읽기 전용으로 조회하는 공유 문서 검색·대화 서비스입니다. 현재 단계는 실제 의료 환경과 무관한
합성 데이터 기능 검증이며, 제품의 성공 기준은 다음 한 줄입니다.

> 왼쪽 패널에서 자연어로 파일 검색 → DB 후보 선택 → 선택한 Revision 범위만 검색 → 중앙 채팅에서 근거와 함께 답변

## 현재 제품 경계

- Frontend: Vue 3 + TypeScript + Vite
- Backend: FastAPI + Pydantic, `backend/src/` 패키지
- 검색: PostgreSQL `llmops.documents`와 `document_chunks` read-only 조회
...
PostgreSQL·MinIO·Neo4j의 read-only 계정과 온프레미스 Ollama 주소만 넣습니다. 실제 값, 내부 주소, 파일 목록은
코드·문서·로그에 기록하지 않습니다.
...
실제 연결 smoke는 합성 데이터가 들어 있는 read-only 저장소에서 수행합니다.
